    ...
    class Categorias extends Controllers{
        ...
        public function getCategorias(){
			if($_SESSION['permisosMod']['r']){
				...
				for ($i=0; $i < count($arrData); $i++) {
					...
					if($_SESSION['permisosMod']['r']){
						$btnView = '<button class="btn btn-info btn-sm " onClick="fntViewInfo('.$arrData[$i]['idtopic'].')" title="Ver categoría"><i class="far fa-eye"></i></button>';
					}
					if($_SESSION['permisosMod']['u']){
						$btnEdit = '<button class="btn btn-primary btn-sm " onClick="fntEditInfo(this,'.$arrData[$i]['idtopic'].')" title="Editar Categoría"><i class="fas fa-pencil-alt"></i></button>';
					}
					if($_SESSION['permisosMod']['d']){
						$btnDelete = '<button class="btn btn-danger btn-sm " onClick="fntDelInfo('.$arrData[$i]['idtopic'].')" title="Eliminar Categoría"><i class="far fa-trash-alt"></i></button>';
					}
			...
			if($_POST){
				if($_SESSION['permisosMod']['d']){
					...
					if($requestDelete == 'ok'){
						...
						$arrResponse = array('status' => false, 'msg' => 'No es posible eliminar una categoría con subcategorias asociadas.');
					}else{
		...
		public function getSelectCategorias(){
			...
			if(count($arrData) > 0 ){
				for ($i=0; $i < count($arrData); $i++) { 
					if($arrData[$i]['status'] == 1 ){
						$htmlOptions .= '<option value="'.$arrData[$i]['idtopic'].'">'.$arrData[$i]['name'].'</option>';
					}
				}
			}
			echo $htmlOptions;
